...
class MARS:
    ...
    def __init__(self, n_variables=None, n_points=None, x=None, y=None, n_candidate_knots=[], n_max_basis_functions=0,
        ...
        if x is not None:
            ...
            # self.x_means = x_scaler.mean_
            # self.x_scales = x_scaler.scale_
            x_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1)).fit(x)
            # x_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1))
            # x_scaler.data_min_ = np.array([-1.99633744, -1.99438408], np.float64)
            # x_scaler.data_max_ = np.array([[1.94359663, 1.94359663], np.float64])
            # x_scaler.scale_ = np.array([0.50762271, 0.5078745], np.float64)
            # x_scaler.min_ = np.array([0.01338622, 0.01289683], np.float64)
            self.x_middles = (x_scaler.data_max_ + x_scaler.data_min_) * 0.5
            self.x_half_ranges = x_scaler.data_range_ * 0.5

            self.x = x_scaler.transform(x)
            # print(self.x_half_ranges)
            # self.x_means = x_scaler.mean_
            # self.x_scales = x_scaler.scale_ [3.93993407 3.93798071]

            # print(self.x)
            # print(x_scaler.data_max_, x_scaler.data_min_, x_scaler.min_)
            # print(x_scaler.data_range_)
            # print(x_scaler.feature_range)
            # print(x_scaler.min_)
    ...
    def predict(self, x_new):
        ...
        # y_scaler.mean_ = self.y_mean
        # y_scaler.scale_ = self.y_scale
        # y = y_scaler.inverse_transform(y)

        return y

    def print_mars_model_latex(self):
        return

    def print_mars_model(self):
        mars_model_str = "f(x)="
        mars_model_str += "{:.6f}".format(self.coefficients[0, 0])
        for ind_bf in range(1, self.n_basis_fn + 1):
            bf = self.basis_fns[ind_bf]
            order = bf.order
            c = self.coefficients[ind_bf, 0]
            if c > 0:
                mars_model_str += "\n+ {:.6f}".format(c)
            else:
                mars_model_str += "\n- {:.6f}".format(-c)

            for ind_k in range(order):
                mars_model_str += "\\left["
                if bf.knot_items[ind_k].sign == 1:
                    mars_model_str += "+\\left("
                else:
                    mars_model_str += "-\\left("

                m = self.x_middles[bf.knot_items[ind_k].index_of_variable]
                h = self.x_half_ranges[bf.knot_items[ind_k].index_of_variable]
                v_ind = bf.knot_items[ind_k].index_of_variable
                if v_ind == 0:
                    if m > 0:
                        mars_model_str += "\\frac{{x-{}}}{{{}}}".format(m, h)
                    else:
                        mars_model_str += "\\frac{{x+{}}}{{{}}}".format(-m, h)
                else:
                    if m > 0:
                        mars_model_str += "\\frac{{y-{}}}{{{}}}".format(m, h)
                    else:
                        mars_model_str += "\\frac{{y+{}}}{{{}}}".format(-m, h)

                k = bf.knot_items[ind_k].knot_value
                if k > 0:
                    mars_model_str += "-{}\\right)".format(k)
                else:
                    mars_model_str += "+{}\\right)".format(-k)
                # if bf.knot_items[ind_k].knot_value>0:
                #     mars_model_str += "x-{})".format(bf.knot_items[ind_k].knot_value)
                # else:
                #     mars_model_str += "x+{})".format(-bf.knot_items[ind_k].knot_value)
                mars_model_str += "\\right]_+"
                # f.write("{}  {}  {:.10f}  {}\n".format(ind_bf, (bf.knot_items[ind_k].index_of_variable + 1),
                #                                        bf.knot_items[ind_k].knot_value, bf.knot_items[ind_k].sign))
        print(mars_model_str)

        # print(type(self.y_scale))
        # print(self.y_scale.dtype)
        # print(self.y_scale.shape)
        #
        # print(type(self.y_mean))
        # print(self.y_mean.dtype)
        # print(self.y_mean.shape)
        #
        # print(type(self.x_scales))
        # print(self.x_scales.dtype)
        # print(self.x_scales.shape)
        #
        # print(type(self.x_means))
        # print(self.x_means.dtype)
        # print(self.x_means.shape)

        # with open(filename, "w") as f:
        #     f.write("{}  {}\n".format(self.n_variables, self.n_basis_fn))
        #     f.write("{:.10f}".format(self.y_mean[0]))
        #     for xm in self.x_middles:
        #         f.write("  {:.10f}".format(xm))
        #     f.write("\n")
        #     f.write("{:.10f}".format(self.y_scale[0]))
        #     for xh in self.x_half_ranges:
        #         f.write("  {:.10f}".format(xh))
        #     f.write("\n")
        #     for i in range(1 + self.n_basis_fn):
        #         f.write("{:.10f}  ".format(self.coefficients[i, 0]))
        #     f.write("\n")
        #     for bf in self.basis_fns:
        #         f.write("{}  ".format(bf.order))
        #     f.write("\n")
        #     for ind_bf in range(1, self.n_basis_fn + 1):
        #         bf = self.basis_fns[ind_bf]
        #         order = bf.order
    ...
    def save_mars_model_to_file(self, filename):
        # print(type(self.y_scale))
        # print(self.y_scale.dtype)
        # print(self.y_scale.shape)
        #
    ...
    def load_mars_model_from_file(self, filename):
        with open(filename, "r") as f:
            vs = f.read().split()
            ind_vs = 0
            self.n_variables = int(vs[ind_vs])
            ind_vs += 1
            self.n_basis_fn = int(vs[ind_vs])
            ind_vs += 1

            self.y_mean = np.zeros((1), np.float64)
            self.y_mean[0] = float(vs[ind_vs])
            ind_vs += 1

            self.x_middles = np.zeros((self.n_variables), np.float64)
            for i in range(self.n_variables):
                self.x_middles[i] = float(vs[ind_vs])
                ind_vs += 1

            self.y_scale = np.zeros((1), np.float64)
            self.y_scale[0] = float(vs[ind_vs])
            ind_vs += 1

            self.x_half_ranges = np.zeros((self.n_variables), np.float64)
            for i in range(self.n_variables):
                self.x_half_ranges[i] = float(vs[ind_vs])
                ind_vs += 1

            # print(self.y_mean, self.x_means)
            # print(self.y_scale, self.x_scales)

            orders = [0] * (self.n_basis_fn + 1)
            self.coefficients = np.zeros((self.n_basis_fn + 1, 1), np.float64)
            for i in range(self.n_basis_fn + 1):
                self.coefficients[i, 0] = float(vs[ind_vs])
                ind_vs += 1
            for i in range(self.n_basis_fn + 1):
                orders[i] = int(vs[ind_vs])
                ind_vs += 1
            for i in range(self.n_basis_fn + 1):
                bf = BasisFunction()
                for o in range(orders[i]):
                    nki = KnotItem(float(vs[ind_vs + 2]), int(vs[ind_vs + 1]) - 1, int(vs[ind_vs + 3]))
                    bf.add_knot_item(nki)
                    ind_vs += 4
    ...
    # def get_static_knots(self):
    #     for i in range(self.n_variables):
    #         self.static_knots[i] = [self.x[0, i]]
    #     for i in range(self.n_variables):
    #         self.get_static_knots_for_variable_i(self.x[:, i], self.static_knots[i])
    #     return
